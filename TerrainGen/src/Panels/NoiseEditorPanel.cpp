...
void NoiseEditorPanel::OnImGuiRender()
{
    ...
    if (ImGui::BeginTabItem("Falloff Settings"))
    {
        ...
        if (s_Props.FalloffMap.Type == (int)NoiseEditorPanelProps::FalloffMapType::RadialGradient)
        {
            ...
            if (ImGui::DragFloat("Radius", &s_Props.FalloffMap.RadialGradient.Radius, 0.01f, 0.01f, 5.0f, "%.3f", ImGuiSliderFlags_AlwaysClamp))
            {
                s_Props.NoiseUpdateFlag = true;
            }
            ImGui::PopID();
...
float NoiseEditorPanel::GetFalloffValue(uint32_t x, uint32_t y)
{
    ...
    case (int)NoiseEditorPanelProps::FalloffMapType::RadialGradient:
    {
        ...
        float radialValue = Enxus::Math::Smoothstep(radius, radius - s_Props.FalloffMap.RadialGradient.Blend, distance);
